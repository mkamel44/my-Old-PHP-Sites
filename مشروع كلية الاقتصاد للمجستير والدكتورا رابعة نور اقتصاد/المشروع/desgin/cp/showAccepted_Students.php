...
$all_rows_count = $db->getCountAccepted_Students();

$products       = $db->getAccepted_Students($offset,$row_count);
		...
		<h2 class="right">
			...
			<script type="text/javascript">
				$("#aa3").on('click', function ()
					{
						$("#main_div").load("addAccepted_Students.php");
					});
			</script>
		</h2>
	</div>
				...
				for($a1 = 0;$a1 < count($products);$a1++){
					...
					<tr <? echo (( $a1 % 2 ) ? "class='odd'" : "" ); ?> >
						<td align="center">
							<h3>
								<?
								echo $products_one->practical_sign;
								?>
							</h3>
						</td>
						<td align="center">
							<h3>
								<?
								echo $products_one->theoretical_sign;
								?>
							</h3>
						</td>
						<td align="center">
							<h3>
								<?
								$mystd = $db->getStudentsByID($products_one->student_id);
						...
						<td align="center">
							<a href="#" val="<? echo $products_one->id; ?>"  key="deleteAccepted_Students.php" onclick="call_click(this);" class="ico del">
								حذف
							</a>
							<a href="#" val="<? echo $products_one->id; ?>" key="updateAccepted_Students.php" onclick="call_click(this);" class="ico edit">
								تعديل
							</a>
						</td>
						<td align="center">
				...
				<div class="right">
					...
					<script type="text/javascript">
						$("#perv_").click(function()
							{
								$("#main_div").load("showAccepted_Students.php",{ offset  : <? echo $offset_perv; ?> , row_count  : <? echo $row_count; ?> });
							});
					</script>
					...
					<script type="text/javascript">
						$("#next_").click(function()
							{
								$("#main_div").load("showAccepted_Students.php",{ offset  : <? echo $offset_next; ?> , row_count  : <? echo $row_count; ?> });
							});
					</script>
				</div>
			</div>
...
<script>
	function call_click(tag_here)
	{
		...
		if(key == "deleteAccepted_Students.php")
		{
		...
		if(key == "updateAccepted_Students.php")
		{
