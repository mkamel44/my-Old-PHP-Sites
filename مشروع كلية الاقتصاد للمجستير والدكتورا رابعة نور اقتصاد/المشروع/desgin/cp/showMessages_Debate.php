...
$all_rows_count = $db->getCountMessages_Debate();

$products       = $db->getMessages_Debate($offset,$row_count);
		...
		<h2 class="right">
			...
			<script type="text/javascript">
				$("#aa3").on('click', function ()
					{
						$("#main_div").load("addMessages_Debate.php");
					});
			</script>
		</h2>
	</div>
				...
				for($a1 = 0;$a1 < count($products);$a1++)
				{
					...
					<tr <? echo (( $a1 % 2 ) ? "class='odd'" : "" ); ?> >
						...
						<td align="center">
							<h3>
								<?
								$dddd = $db->getMessageByID($products_one->messages_id);
								echo $dddd->title;
								?>
							</h3>
						</td>
						...
						<td align="center">
							<a href="#" val="<? echo $products_one->id; ?>"  key="deleteMessages_Debate.php" path="<? echo $products_one->pdf; ?>" onclick="call_click(this);" class="ico del">
								حذف
							</a>
							<a href="#" val="<? echo $products_one->id; ?>" key="updateMessages_Debate.php" onclick="call_click(this);" class="ico edit">
								تعديل
							</a>
						</td>
						<td align="center">
				...
				<div class="right">
					...
					<script type="text/javascript">
						$("#perv_").click(function()
							{
								$("#main_div").load("showMessages_Debate.php",{ offset  : <? echo $offset_perv; ?> , row_count  : <? echo $row_count; ?> });
							});
					</script>
					...
					<script type="text/javascript">
						$("#next_").click(function()
							{
								$("#main_div").load("showMessages_Debate.php",{ offset  : <? echo $offset_next; ?> , row_count  : <? echo $row_count; ?> });
							});
					</script>
				</div>
			</div>
...
<script>
	function call_click(tag_here)
	{
		...
		if(key == "deleteMessages_Debate.php")
		{
		...
		if(key == "updateMessages_Debate.php")
		{
