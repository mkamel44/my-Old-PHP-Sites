...
<div class="box">
	...
	<div class="table">
		...
		if($MembersID != "")
		{
			$std = $db->getMessageByID($MembersID);
		}
						...
						$("#addshop2").on('click', function ()
							{
								...
								if($("#depa_id").val() != "" && $("#university_no").val() != "" && $("#name").val() != "" && $("#title").val() != ""  )
								{
									$("#addshop2").attr("disabled", true);

									$("#main_div").load("updateMessages.php",{  depa_id : $("#depa_id").val(),university_no : $("#university_no").val(),name : $("#name").val(),title: $("#title").val(),supervisor_name : $("#supervisor_name").val(),supervisor_name1 : $("#supervisor_name1").val(),registration_date : $("#registration_date").val(),grant_date : $("#grant_date").val(),comments_data : $("#comments_data").val(), MembersID : <? echo $MembersID; ?>, update : "done"  });
								}
								else
								{
