...
if($the_date != "")
{
	$msgs = $db_messages->getAllMessages_DebateByDate($the_date);

}
else
if($depa_id != 0)
{
	$msgs = $db_messages->getAllMessages_DebateByDepaID($depa_id);

}
else
{
	$msgs = $db_messages->getAllMessages_Debate();
}
				...
				$("#dooo").click(function()
					{
						if($("#the_date").val() != "")
						{
							$("#main_div").load("getMessages_ok.php",{ the_date : $("#the_date").val() });
						}
						else
						{
							...
							$("#depa_id").change(function()
								{
									...
									$("#main_div").load("getMessages_ok.php",{depa_id : selectedValue});
...
<div class="row">
	...
	for($i=-1;$i < count($msgs);){
		...
		<div class="col-m4-12" style="padding: 5px;width: 340px">
			...
			$last_msg = $msgs[$i];
			if( $last_msg->id != ""){
				$last_msg_o = $db_messages->getMessageByID($last_msg->messages_id);
				include("message_ok.php");
			}
